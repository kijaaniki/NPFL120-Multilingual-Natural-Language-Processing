...
l_data = np.asarray(Lang_Data)[:,4:]
wals = (np.asarray(Lang_Data)[:,0]).tolist()

Meta = []

for col in range(l_data.shape[1]):
    u = (np.unique(l_data[:,col])).tolist()
    D = {}
...
for row in range(l_data.shape[0]):
    line = l_data[row]
    C = []
